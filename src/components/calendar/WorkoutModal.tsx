...
export function WorkoutModal({ date, onClose, onSave }: WorkoutModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-ink/35 p-3 backdrop-blur-sm">
      <div className="mx-auto flex h-full max-w-2xl flex-col overflow-hidden rounded-lg bg-[#f7fbf8] shadow-soft">
        <div className="flex items-center justify-between border-b border-ink/10 bg-white p-4">
          <div>
            <p className="text-sm font-bold text-mint">{date}</p>
            <h2 className="text-xl font-black">운동 일지 작성</h2>
          </div>
          <button type="button" aria-label="닫기" title="닫기" onClick={onClose} className="p-2">
            <X size={22} />
          </button>
        </div>
        ...
        <div className="grid flex-1 gap-4 overflow-y-auto p-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="grid gap-2 text-sm font-semibold text-ink">
              ...
              <select
                value={condition}
                onChange={(event) => setCondition(event.target.value as WorkoutSession['condition'])}
          ...
          <div className="grid gap-3">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setExercises((items) => [...items, emptyExercise()])}
                className="flex min-h-10 items-center gap-2 rounded-lg border border-mint px-3 text-sm font-black text-mint"
              >
                <Plus size={16} />
                추가
              </button>
            ...
            {exercises.map((exercise) => (
              <div key={exercise.id} className="rounded-lg border border-ink/10 bg-white p-3">
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <Field
                    ...
                    onChange={(event) => updateExercise(exercise.id, { equipment: event.target.value })}
                  />
                </div>
                <div className="mt-3 grid gap-2">
                  {exercise.sets.map((set, index) => (
                    <div key={set.id} className="grid grid-cols-[2rem_1fr_1fr] items-end gap-2">
                      <span className="pb-3 text-sm font-black text-ink/60">{index + 1}</span>
                      <Field
                        label="kg"
                        type="number"
                        value={set.kg}
                        onChange={(event) => updateSet(exercise.id, set.id, { kg: Number(event.target.value) })}
                      />
                      <Field
                        label="회수"
                        type="number"
                  ...
                  <button
                    ...
                    onClick={() =>
                      updateExercise(exercise.id, {
                        sets: [...exercise.sets, { id: crypto.randomUUID(), kg: 0, reps: 0 }]
                      })
                    }
                    className="min-h-10 rounded-lg bg-[#edf6f2] text-sm font-black text-mint"
                  >
                    세트 추가
                  </button>
                </div>
                {exercises.length > 1 ? (
                  <button
                    type="button"
                    onClick={() => setExercises((items) => items.filter((item) => item.id !== exercise.id))}
                    className="mt-3 flex min-h-10 items-center gap-2 text-sm font-black text-coral"
                  >
                    <Trash2 size={16} />
                    운동 삭제
                  </button>
        ...
        <div className="border-t border-ink/10 bg-white p-4">
          <button type="button" onClick={save} className="min-h-12 w-full rounded-lg bg-mint font-black text-white">
            저장
          </button>
        </div>
      </div>
