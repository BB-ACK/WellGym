  ...
  persist(
    (set, get) => ({
      ...
      addWorkout: (workout) => {
        ...
        set((state) => ({
          workouts: [...state.workouts, { ...workout, id: id('workout'), color, pendingSync: !navigator.onLine }]
        }))
      },
      addInbody: (entry) => {
        set((state) => ({
          inbodyEntries: [{ ...entry, id: id('inbody'), pendingSync: !navigator.onLine }, ...state.inbodyEntries]
        }))
      },
      markSynced: () =>
        set((state) => ({
    ...
    { name: 'wellgym-offline-store', version: 1 }
  )
)
